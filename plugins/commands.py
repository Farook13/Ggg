...
async def start(client, message):
    ...
    if len(message.command) != 2:
        ...
        m = await message.reply_sticker("CAACAgUAAxkBAAEbyxNls1_-FloyOUHa4IdQGWNZAg3G5AACrAsAAqo5wFT5az_ATquwjzQE") 
        await asyncio.sleep(2)
        await message.reply_photo(photo=random.choice(PICS), caption=START_MESSAGE.format(user=message.from_user.mention, bot=client.mention), reply_markup=InlineKeyboardMarkup(buttons), parse_mode=enums.ParseMode.HTML)
        return await m.delete()
        
        channels = ['-1001882687525', '1195733628', '-1001786807217']
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, message):
            ...
            for invite_link in invite_links:
                btns.append(InlineKeyboardButton("Join Our Channel", url=invite_link))
            
            if message.command[1] != "subscribe":
                try:
                    kk, file_id = message.command[1].split("_", 1)
                    pre = 'checksubp' if kk == 'filep' else 'checksub' 
                    btns.append(InlineKeyboardButton("⟳ Tʀʏ Aɢᴀɪɴ", callback_data=f"{pre}#{file_id}"))
                except (IndexError, ValueError):
                    btns.append(InlineKeyboardButton("⟳ Tʀʏ Aɢᴀɪɴ", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}"))
            
            keyboard = InlineKeyboardMarkup([[btns[0]], [btns[1]], [btns[2]]])  # Each button in a separate row
            await message.reply_text("Join our channels:", reply_markup=keyboard)
